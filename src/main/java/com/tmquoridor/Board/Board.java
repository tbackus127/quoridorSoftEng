...
import com.tmquoridor.Util.DebugOut;
...
public class Board {
  ...
  private static final Coord[] startPos = { new Coord(4, 0), new Coord(4, 8), new Coord(0, 4), new Coord(8, 4) };
  ...
  /**
   * Default contructor Defaults to two players
   */
  public Board() {
    this(2);
  }
  ...
  public HashSet<Wall> getBlockingWalls(int pid, ArrayList<Coord> path) {
    ...
    for (Coord c : path) {
      ...
      for (int i = 0; i < numOfPlayers && !canPlace; i++) {
        if (isPlayerKicked(i) || pid == i) continue;
  ...
  public HashSet<Coord> getLegalMoves(int pid) {
    ...
    if (c == null) System.err.println("getLegalMoves(int):c is null! PID=" + pid);
  ...
  public boolean isBlocked(Coord src, Direction dir) {
    ...
    // Check Board bounds
    if (src.getY() <= 0 && dir == Direction.NORTH) return true;
    if (src.getY() >= 8 && dir == Direction.SOUTH) return true;
    if (src.getX() <= 0 && dir == Direction.WEST) return true;
    if (src.getX() >= 8 && dir == Direction.EAST) return true;
  ...
  /**
   * Moves a player to a destination. Does not check for collisions, so an upper layer of control is needed.
   * 
  ...
  public void placeWall(int pid, Coord pos, Orientation ort) {
    if (wallsLeft[pid] <= 0) System.err.println("Placed too many walls! (Player: " + pid);
    wallsLeft[pid]--;
    placeWall(pos, ort);
  }
  ...
  private MoveTrace getLegalMoves(MoveTrace mt, Coord curr) {
    ...
    for (Direction dir : Direction.values()) {
      ...
      /*
       * // If there is a player in this direction if(pid >= 0) { // If they haven't been seen yet, add their legal
       * moves to this if(!mt.isSeen(pid)) { mt.addPlayer(pid); mt.add(getLegalMoves(mt, c2)); } // If no player and not
       * blocked, add it } else if(!isBlocked(curr, dir)) { mt.addMove(c2); }
       */
