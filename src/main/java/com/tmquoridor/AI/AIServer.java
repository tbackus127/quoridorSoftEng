...
import com.tmquoridor.Servers.ManualInputServer;
    ...
    if (pidWithShortestPath == pid) {
      ...
      if (!noWallsLeft) {
        ...
        for (Wall w : blockingWalls) {
          ...
          if (theoreticalPath.size() - currPathLength >= wallCounterThreshold
              && theoreticalPath.size() > worstPathLength) {
        ...
        if (currPathLength <= remainingPathThreshold || rand.nextDouble() <= randomWallChance) {
          ...
          for (Wall w : blockingWalls) {
            ...
            if (theoreticalPath.size() > bestWallPathLength
                && theoreticalDelta + (7 - currPathLength) >= attackDeltaThreshold) {
