...
/**
 * This class handles manual player input to the client.
 */
public class ManualInputServer {
  ...
  public void run() {
    try {
      ...
      System.out.println("Server now accepting connections " + "on port " + port);
  ...
  /*
   * This takes a string in the format of "t c r" where t is the type of move being either m = move, v = verticle wall
   * or h = horizontal wall. It then returns the String in the correct format for the protocol
   */
  ...
  public void sendMove(PrintStream cout, Scanner console) {
    ...
    if (moveType.equals("m")) {
      ...
      System.out.print("Enter \"h\" for a horizontal wall or \"v\" for" + " a vertical wall: ");
      unwrappedMessage = console.next() + " ";
