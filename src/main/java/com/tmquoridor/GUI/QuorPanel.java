...
public class QuorPanel extends JPanel {
  ...
  /** Tile color */
  private static final Color COLOR_TILE[] = { new Color(128, 0, 0), new Color(128, 128, 128), new Color(0, 0, 128) };
  ...
  /** Path colors */
  private static final Color COLOR_PATH[] = { new Color(0, 255, 0), new Color(255, 255, 0), new Color(0, 255, 255),
      new Color(255, 0, 255) };
  ...
  private void paintResults(Graphics g) {
    ...
    g2.drawString(resHeaderString, (640 - g2.getFontMetrics(fontResHeaders).stringWidth(resHeaderString)) / 2,
        MARGIN_RESLINE * 2);
    ...
    for (int i = 1; i < matchResults.length; i++) {
      if (matchResults.length > 2) resPlayerList += ",";
      if (i == matchResults.length - 1) resPlayerList += " and";
      resPlayerList += " " + matchResults[i];
    }
    g2.drawString(resPlayerList, (640 - g2.getFontMetrics(fontResText).stringWidth(resPlayerList)) / 2,
        MARGIN_RESLINE * 6);
  ...
  private void paintPawns(Graphics g) {
    ...
    for (int i = 0; i < board.getTotalPlayers(); i++) {
      ...
      if (board.isPlayerKicked(i)) continue;
  ...
  private void paintPaths(Graphics g) {
    ...
    for (int pid = 0; pid < board.getTotalPlayers(); pid++) {
      ...
      if (board.isPlayerKicked(pid)) continue;
